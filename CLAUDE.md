...
### Browser URL polling (v1)

v1 adds an opt-in `BrowserURLPoller` for browser-hosted meetings that the existing `MeetingActivitySnapshot.browserURL` path doesn't already cover (private windows, profiles, PWA-shaped tabs). All flags live under `auto_capture.browser_url_polling.{chrome,edge,brave,arc,safari}` and default to off — v0 behaviour is unchanged. The poller is owned by `AutoCaptureCoordinator` and bootstrapped from `coordinator.start()` via a `BrowserURLPollerFactory`; tests can pass `browserURLPollerFactory: nil` to opt out. While at least one browser is enabled, a 1Hz watchdog calls `AudioProcessAttributionCollector.activeInputProcesses()`. Only when one of the enabled browsers owns a microphone input device does the poller invoke `NSAppleScript` at 0.5s cadence — when the mic is released, the poller drops back to the watchdog and stops issuing AppleScript calls. URL matching against `meet.google.com`, `*.zoom.us/wc/*`, `teams.microsoft.com`, `teams.live.com`, `teams.cloud.microsoft`, and `app.webex.com` happens in `BrowserMeetingURLMatcher`. Matches flow into the existing `AutoCaptureCoordinator.handle(_:)` so the v0 state machine handles them identically to a v0 tier-C detection. macOS Automation permission is probed non-interactively via `AEDeterminePermissionToAutomateTarget` (with `askUserIfNeeded == false`), and a denied target is surfaced via a persistent banner in the Auto-Capture settings pane plus a new onboarding step. Onboarding schema bumped from v4 to v5; the new `Step.automation = 7` is appended to the meeting flow so existing `currentStep ≤ 6` users resume cleanly. See `docs/decisions/0003-applescript-vs-extension-browser-detection.md` for the rationale.

### PWA discovery (v2)

v2 surfaces installed Progressive Web Apps as first-class entries in the Auto-Capture allowlist. The new `PWADiscovery` module performs a synchronous filesystem scan: Chrome PWAs are picked up from `~/Applications/Chrome Apps.localized/` (bundle IDs of the form `com.google.Chrome.app.<32-char id>`), Safari Web Apps from both `/Applications/` and `~/Applications/` (bundle IDs prefixed `com.apple.Safari.WebApp.`). The scan is best-effort about start URLs: for each Chrome PWA it walks `~/Library/Application Support/Google/Chrome/<profile>/Web Applications/` looking for both the legacy `_crx_<appid>/manifest.json` and modern `<appid>/manifest.json` layouts. Safari Web Apps expose only the bundle's URL scheme, so `startURL` is often `nil` — toggles still work because the wiring keys off bundle ID, not URL. Discovered entries live under `auto_capture.pwa.cached_entries`; per-entry toggle state lives under `auto_capture.pwa.enabled`. Toggling a PWA on in the new "PWAs (v2)" Settings section adds its bundle ID to `auto_capture.allowed_app_bundle_ids` so the existing `AutoCaptureCoordinator.isAllowed` check passes — no new state-machine tier required. Refresh runs off the main actor (`Task.detached(priority: .utility)`) and marshals results back via `MainActor.run`, then prunes stale toggle entries whose bundles disappeared since the previous scan. Refresh fires on app launch (from `configureAutoCaptureCoordinator`) and whenever the user clicks the Refresh button or reopens the Auto-Capture pane.

### Stop detection (v2.1)

v2.1 closes the loop for browser-hosted and PWA-hosted meetings by adding auto-stop. A new `BrowserMicReleaseMonitor` under `AutoCapture/` installs a CoreAudio HAL listener (`kAudioProcessPropertyIsRunningInput`) on every per-process AudioObject owned by the watched browser-family bundle, plus a global `kAudioHardwarePropertyProcessObjectList` listener that reattaches when helper processes spawn or restart. PWAs at the HAL layer report their parent browser's bundle, so the monitor rolls PWA bundles up via `parentBrowserBundleID(for:)` before installing listeners (`com.apple.Safari.WebApp.*` → `com.apple.Safari`, `com.google.Chrome.app.*` → `com.google.Chrome`, etc.). When the parent bundle drops out of `AudioProcessAttributionCollector.activeInputProcesses()`, an 8-second debounce starts; if the bundle is still absent after the debounce re-check, the monitor invokes `onCallEnded(bundleID:)`. `AutoCaptureCoordinator` accepts a new `recordingStopper: (() -> Void)?` (wired to `MuesliController.stopMeetingRecording()` and guarded by `isMeetingRecording()`) plus a `BrowserMicReleaseMonitorFactory`. On a successful auto-started recording for an eligible browser bundle, the coordinator calls `monitor.beginWatching(bundleID:)`; when the monitor fires, `handleAutoStop(bundleID:)` checks the recording bundle matches (directly or via `parentBrowserBundleID`) and that `config.autoStopEnabled` is true, then calls `recordingStopper?()` and transitions to `.idle` with `AutoCaptureDecisionReason.autoStopped`. The master toggle is `auto_capture.auto_stop_enabled` (default `true`, snake-case JSON, decodes-without-key for back-compat). Settings adds one experimental toggle ("Auto-stop when call ends (experimental)") in the existing Behaviour section; the first-run modal appends one disclosure sentence describing auto-stop as experimental. The Browser Auto-Capture onboarding step is removed (its 1 Hz status poll raced the interactive request, flipping the per-browser pill Request → Denied → Request); `OnboardingProgress.currentSchemaVersion` bumped from 5 → 6, with saved progress at the removed step reset to a fresh start. Native meeting-app auto-stop is out of v2.1 scope — v0's `isRecordingNowProbe` path still owns Zoom/Teams/Webex desktop. Known false-negatives (concurrent mic tabs in the same browser, warm streams in Slack-huddle-minimised, AirPods migration within the debounce window) are documented in ADR 0009.
